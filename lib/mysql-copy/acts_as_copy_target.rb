  ...
  module ActsAsCopyTarget
    ...
    module CopyMethods
      ...
      def copy_from path_or_io, options = {}
        ...
        table_string = table.delete! '``'
        io = Tempfile.new("copy_binary", :encoding => 'ascii-8bit')

        columns_list = columns_list.map{|c| options[:map][c.to_s] } if options[:map]
        columns_string = columns_list.size > 0 ? "\(#{columns_list.join(', ')}\)" : ""
        connection.execute("LOAD DATA LOCAL INFILE '#{io.path}' INTO TABLE #{table_string}#{columns_string}") 
...
#         connection.raw_connection.put_copy_end
    ...
    module ClassMethods
      def acts_as_copy_target
        extend MysqlCopy::ActsAsCopyTarget::CopyMethods
      end
    end
  end
end

ActiveRecord::Base.send :include, MysqlCopy::ActsAsCopyTarget
